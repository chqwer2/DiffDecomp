...
def get_im(im_path='clevr_im_10.png', resolution=64):
    ...
    im[..., 0] = im[..., 0]/3 + im[..., 1]/3 + im[..., 2]/3
    im[..., 1] = im[..., 0]
    im[..., 2] = im[..., 0]
    
    print("get sample im stat:", im.shape, np.mean(im), np.std(im))
...
# Use for sample image
def gen_image_and_components(model, gd, slices=None, separate=False, 
                             num_components=4, sample_method='ddim', im_path='clevr_im_10.png', 
                             batch_size=1, image_size=64, device='cuda', model_kwargs=None, 
                             num_images=4, desc='', save_dir='', dataset='clevr', mode="vanilla"):
    ...
    # generate imgs
    for i in range(num_images):
        all_samples = [(orig_img - orig_img.min())/ (orig_img.max() - orig_img.min())] # normalize
        # individual components
        for j in range(num_components):
            model_kwargs['latent_index'] = j   # latent index indicates the 
            ...
            print("sample stat:", sample.shape, sample.min(), sample.max(), sample.std(), sample.mean())
            sample = (sample - sample.min()) / (sample.max() - sample.min()) # normalize
            all_samples.append(sample)
        ...
        print("recon sample stat:", sample.shape, sample.min(), sample.max(), sample.std(), sample.mean())
        all_samples.append(sample)
        sample = (sample - sample.min()) / (sample.max() - sample.min()) # normalize
        all_samples.append(sample)
        ...
        # save row
        # grid.mul(255).add_(0.5).clamp_(0, 255)
...
def get_gen_images(model, gd, sample_method='ddim', im_path='clevr_im_10.png', 
                   latent=None, batch_size=1, image_size=64, device='cuda', model_kwargs=None, 
                   num_images=4, desc='', save_dir='', free=False, guidance_scale=10.0, 
                   dataset='clevr', separate=False, mode='vanilla'):
    orig_im = get_im(im_path=im_path, resolution=image_size)  # 64
    slices  = th.tensor([50], device=device)
    # [None, :, :, :].contiguous().cuda()
    ...
    gen_image_and_components(gen_model, gd, slices=slices, separate=separate, num_components=model.num_components, 
                             sample_method=sample_method, im_path=im_path, batch_size=batch_size, image_size=image_size, device=device, 
                             model_kwargs=model_kwargs, num_images=num_images, desc=desc, save_dir=save_dir, dataset=dataset, mode=mode)
...
def combine_components_add(model, gd, sample_method='ddim', im1='clevr_im_10.png', 
                           im2='clevr_im_25.png', device='cuda', num_images=4, model_kwargs={}, 
                           desc='', save_dir='', dataset='clevr', image_size=64, mode="vanilla"):
    """Combine by adding components together
    """
    assert sample_method in ('ddpm', 'ddim')
