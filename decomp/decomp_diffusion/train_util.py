...
def run_loop(model, gd, data, save_desc, lr=1e-3, start_epoch=0, epoch_block=10000, num_its=20, p_uncond=0.0, 
             default_im='im_10.png', ddim_gd=None, latent_orthog=False, ema_rate=0.9999, dataset='clevr', 
             downweight=False, image_size=64, use_dist=False):
    ...
    optimizer = th.optim.AdamW(model.parameters(),  # W?
                                lr=lr)
                                # weight_decay = 1e-8)
    ...
    for epoch in range(start_epoch, start_epoch + total_epochs):
        ...
        loss = gd.training_losses(model, batch, t, s=slices, model_kwargs=model_kwargs, latent_orthog=latent_orthog, downweight=downweight)
        loss = loss.mean() 
        ...
        if epoch % epoch_block == 0:
            print(f'sample img at {epoch} epochs')
            print(f'Saving images to {imgs_save_dir}')
            # ema
            get_gen_images(model, ddim_gd, im_path=default_im, desc=str(epoch), 
                           save_dir=imgs_save_dir, free=free, dataset=dataset, 
                           sample_method='ddim', num_images=1, image_size=image_size, mode="vanilla")
            
            get_gen_images(model, ddim_gd, im_path=default_im, desc=str(epoch)+"_revise", 
                           save_dir=imgs_save_dir, free=free, dataset=dataset, 
                           sample_method='ddim', num_images=1, image_size=image_size, mode="revise")
            
            get_gen_images(model, ddim_gd, im_path=default_im, desc=str(epoch)+"_revise_with_tau", 
                            save_dir=imgs_save_dir, free=free, dataset=dataset, 
                            sample_method='ddim', num_images=1, image_size=image_size, mode="revise_with_tau")

            print(f'epoch: {epoch}/{epoch_block} loss:', loss)
