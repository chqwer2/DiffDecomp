...
def main():
    ...
    save_desc = f'results/{comment}_{model_desc}_{dataset}_size_{image_size}_{num_images}_{predict_desc}_emb_{args.emb_dim}_{args.loss_mode}' +\
                f"_fusion_{args.components_fusion}" 
    ...
    if log_folder is None:
        log_folder = 'results/logs_' + save_desc
    ...
    model.to(dist_util.dev(use_dist))
    ...
    run_loop(model, gd, data, save_desc, start_epoch=start_epoch, epoch_block=args.epoch_block, num_its=args.num_its, p_uncond=p_uncond, 
             default_im=default_im, latent_orthog=args.latent_orthog, dataset=dataset, downweight=downweight, image_size=image_size, use_dist=use_dist)
