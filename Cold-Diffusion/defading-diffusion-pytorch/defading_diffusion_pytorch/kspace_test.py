...
def undersample_mri(raw_mri, _MRIDOWN, _SNR):
    ...
    ff = create_mask_for_mask_type(mask_type_str,
                                   [center_fraction],
                                   [MRIDOWN])  ## 0.2 for MRIDOWN=2, 0.1 for MRIDOWN=4, 0.04 for MRIDOWN=8
...
def visualize_mri_undersampling(image_path, _MRIDOWN, _SNR):
    # Load the image
    raw_image = np.array(Image.open(image_path).convert('L'))  # Convert to grayscale
    raw_image = cv2.resize(raw_image, (240, 240), interpolation=cv2.INTER_LINEAR)
    # Resize to 240x240

    # Run the undersample_mri function
    noisy_kspace, noisy_mri, kspace, mri, mask = undersample_mri(raw_image, _MRIDOWN, _SNR)

    # Convert tensors to numpy arrays for visualization
    mri_np = mri.numpy()
    mask_np = mask.numpy()
    kspace_np = torch.abs(kspace).numpy()  # Magnitude for visualization
    noisy_mri_np = noisy_mri.numpy()

    # Plot the results
    plt.figure(figsize=(12, 8))

    plt.subplot(2, 3, 1)
    plt.imshow(mri_np.squeeze(), cmap='gray')
    plt.title("Original MRI")
    plt.axis('off')

    plt.subplot(2, 3, 2)
    print("mask_np = ", mask_np.shape)
    ones = np.ones_like(mri_np) * mask_np

    plt.imshow(ones.squeeze(), cmap='gray')
    plt.title("Sampling Mask")
    plt.axis('off')

    plt.subplot(2, 3, 3)
    plt.imshow(np.log(np.abs(kspace_np).squeeze() + 1e-6), cmap='gray')  # Log for better visualization
    plt.title("Original K-space (Log Scale)")
    plt.axis('off')

    plt.subplot(2, 3, 4)
    plt.imshow(noisy_mri_np.squeeze(), cmap='gray')
    plt.title("Noisy MRI")
    plt.axis('off')

    plt.subplot(2, 3, 5)
    plt.imshow(np.log(np.abs(noisy_kspace.numpy()).squeeze() + 1e-6), cmap='gray')
    plt.title("Noisy K-space (Log Scale)")
    plt.axis('off')
...
if __name__ == "__main__":
    path = "/Users/haochen/Documents/GitHub/DiffDecomp/Cold-Diffusion/generation-diffusion-pytorch/defading_diffusion_pytorch/assets/img.png"
    visualize_mri_undersampling(path, _MRIDOWN="4X", _SNR=0)
